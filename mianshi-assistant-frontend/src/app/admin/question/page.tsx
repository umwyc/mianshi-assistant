...
const QuestionAdminPage: React.FC = () => {
    ...
    // 是否显示批量向题库添加题目弹窗
    const [batchAddQuestionsToBankModalVisible
        , setBatchAddQuestionsToBankModalVisible] =
        useState<boolean>(false);
    // 是否显示批量从题库删除题目弹窗
    const [batchRemoveQuestionsFromBankModalVisible
    ...
    const handleBatchDelete = async (
        ...
        const hide = message.loading("正在删除");
        if (!questionIdList || questionIdList.length === 0) return;
        try {
